...
select
    *,
    (select json_agg(tag.name) from tag_image join tag on tag_image.tag = tag.id where "image" = "image".id) as tags,
...
create view tag_with_last_imageid as
 select
    tag.name,
    count(*) as images,
    max(image)
 from 
    tag_image
    join tag on tag.id = tag_image.tag
    join image on image.id = tag_image.image
 group by 
    tag.name
 order by 
    images desc;
